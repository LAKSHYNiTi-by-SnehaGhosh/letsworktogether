...
export default async function BillingDashboard() {
  ...
  try {
    ...
    console.error("Clerk auth error in BillingDashboard:", authError);
    redirect("/sign-in");
  }
  ...
  try {
    // Ensure Clerk user is synced to PostgreSQL database
    await ensureUserSynced(clerkUser);

    const primaryEmail = clerkUser.emailAddresses?.[0]?.emailAddress || "";
    
    // Find user by ID first, fallback to email
    ...
    const aiTotalLimit = user?.aiTotalLimit ?? 50;
    ...
    const usagePercentage = Math.min(100, Math.round((aiUsageCount / Math.max(1, aiTotalLimit)) * 100));
            ...
            <div className="rounded-xl border border-border bg-card p-6 shadow-sm">
              ...
              <div className="space-y-2 mb-2">
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-medium">{aiUsageCount} / {isPro ? "∞" : aiTotalLimit}</span>
                </div>
    ...
    console.error("Error rendering BillingDashboard:", error);
    return (
      <div className="max-w-5xl mx-auto space-y-8">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">Billing & Subscriptions</h1>
